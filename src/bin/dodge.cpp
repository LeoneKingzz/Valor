...
bool dodge::able_dodge(RE::Actor* a_actor)
{
	...
	if (settings::bZUPA_mod_Check) {
		...
		// auto magicTarget = a_actor->AsMagicTarget();
		if (!a_actor->IsInKillMove() /*&& !CombatTarget->IsInKillMove()*/ && !CombatTarget->AsActorState()->IsBleedingOut() && CTMagicTarget == 0.0 && !magicTarget->HasEffectWithArchetype(RE::EffectArchetypes::ArchetypeID::kDemoralize)
